...
- **Archivos ZIP**: se acepta subir un `.zip` con un estudio (carpetas con `.dcm` y/o imágenes/PDF). Al procesarlo: cada archivo interno se guarda **individualmente** en los `documents` de OpenEMR (con su registro en `form_imaging_report_images`) y, si el checkbox "Además sube al servidor PACS" está activado, también se sube al PACS siguiendo el **mismo camino por archivo que la subida suelta**: cada `.dcm` nativo se sube individualmente (`uploadNativeDicom`) y se fuerza su reasignación al estudio del pedido (`modifyInstance`), de modo que todas las series de todas las carpetas del ZIP quedan agrupadas en un único estudio; las imágenes/PDF se convierten a DICOM con ese mismo UID de estudio. El ZIP comprimido en sí nunca se envía al PACS.
