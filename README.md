...
- **ZIP archives**: uploading a `.zip` containing a study (folders with `.dcm` and/or images/PDF) is supported. When processed, every internal file is saved **individually** to OpenEMR `documents` (with its own `form_imaging_report_images` record) and, if the "Also upload to PACS server" checkbox is enabled, also uploaded to the PACS following the **same per-file path as a standalone upload**: each native `.dcm` is uploaded individually (`uploadNativeDicom`) and force-reassigned to the order's study (`modifyInstance`), so all series from every folder in the ZIP end up grouped under a single study; images/PDF are converted to DICOM with that same study UID. The compressed ZIP itself is never sent to the PACS.
