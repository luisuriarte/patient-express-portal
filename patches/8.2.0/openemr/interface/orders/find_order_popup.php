        ...
        <?php if (!empty($_REQUEST['bn_search']) || !empty($_REQUEST['bn_grpsearch'])): ?>
            ...
            if ($filter_ptn !== '' && $ord === 'ord') {
                if ($filter_ptn === 'imaging') {
                    ...
                    if ($imageRootId > 0) {
                        $whereClauses[] = "(pt.procedure_type_name = 'imaging' OR pt.parent IN (SELECT procedure_type_id FROM procedure_type WHERE parent = ? AND activity = 1))";
                        $params[] = $imageRootId;
                    } else {
            ...
            // 4. Text search (empty term '%%' returns all matching filters above)
